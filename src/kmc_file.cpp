...
bool CKMCFile::ReadParamsFrom_prefix_file_buf(uint64 &size, open_mode _open_mode)
{
	...
	if (kmc_version == 0x200)
	{
		...
		uint32 lut_area_size_in_bytes32 = uint32(lut_area_size_in_bytes);
		asserta(uint64(lut_area_size_in_bytes32) == lut_area_size_in_bytes);
		fseek(file_pre, 4 + lut_area_size_in_bytes32 + 8, SEEK_SET);
